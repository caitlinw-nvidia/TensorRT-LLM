...
pytestmark = pytest.mark.skipif(
    not torch.cuda.is_available(), reason="CUDA is required for stream tests"
)
...
def test_explicit_two_stream_fork_join_is_cuda_graph_safe():
    ...
    joined_output = torch.empty((), device="cuda")
    branch_streams = []
    ...
    with torch.cuda.stream(parent), with_multi_stream(True):
        ...
        torch.add(output0.sum(), output1.sum(), out=joined_output)
    ...
    torch.testing.assert_close(joined_output, output0.sum() + output1.sum())
