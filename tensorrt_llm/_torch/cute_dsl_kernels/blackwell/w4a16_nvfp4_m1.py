...
if _CUTE_AVAILABLE:
    ...
    def ld_global_nc_v4_u32(
        ...
        """Load 128 bits through the non-coherent global-memory cache."""
    ...
    class _DenseGemmW4A16CuteM1Jit:
        ...
        def _dot16(
            ...
            a0, a1, a2, a3 = ld_global_nc_v4_u32(x_ptr)
            b0, b1, b2, b3 = ld_global_nc_v4_u32(x_ptr + Int64(16))
            acc = Float32(0.0)
        ...
        def kernel(
            ...
            if warp_idx < Int32(self.tile_n) and row < Int32(self.n):
                ...
                while k16_block < cols_blocks:
                    ...
                    scale_offset = self._swizzled_e4m3_offset(row, k16_block, sf_cols)
                    scale = cvt_e4m3_to_f32_via_f16(
                        _ld_global_nc_u8(sf_base + scale_offset)
                    )
                    q0, q1 = ld_global_nc_v2_u32(w_row_base + Int64(k0 // Int32(2)))
                    weights0 = self._decode8(q0)
                    weights1 = self._decode8(q1)
                    partial = self._dot16(
                        x_base + Int64(k0) * Int64(2), *weights0, *weights1
                    )
                    acc += partial * scale
                    k16_block += Int32(32)
...
class DenseGemmW4A16CuteM1Kernel:
    ...
    def __call__(
        ...
        *,
        ...
        w_blockscale_u8 = (
            w_blockscale_swizzled_u8.view(torch.uint8)
            if w_blockscale_swizzled_u8.dtype != torch.uint8
            else w_blockscale_swizzled_u8
        )
        compiled(x.reshape(k), w_fp4_u8, w_blockscale_u8, w_alpha.reshape(1), out.reshape(n))
        return out
...
class W4A16NVFP4CuteM1Runner(TunableRunner):
    ...
    def forward(
        ...
        **kwargs,
        ...
        del kwargs
        x, w_fp4, w_blockscale_swizzled_u8, w_alpha = inputs
        kernel = _get_kernel()
        if do_preparation:
            ...
            for candidate in self.get_valid_tactics(inputs, OptimizationProfile()):
                kernel(x, w_fp4, w_blockscale_swizzled_u8, w_alpha, out=out, tactic=candidate)
            return out
        return kernel(x, w_fp4, w_blockscale_swizzled_u8, w_alpha, tactic=tactic)
...
def _w4a16_nvfp4_cute_m1_gemv_tunable(
    ...
    inputs = [x, w_fp4, w_blockscale_swizzled_u8, w_alpha]
...
def _(
    ...
    del w_blockscale_swizzled_u8, w_alpha
...
def w4a16_nvfp4_cute_m1_gemv(
    ...
    *,
    ...
    if out is None:
        return _w4a16_nvfp4_cute_m1_gemv_tunable(
            x, w_fp4, w_blockscale_swizzled_u8, w_alpha
        )
    return _get_kernel()(x, w_fp4, w_blockscale_swizzled_u8, w_alpha, out=out)
